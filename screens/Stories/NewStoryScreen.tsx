...
// function component for NewStoryScreen
function NewStoryScreen(props: AppScreenProps<"NewStoryScreen">) {
  // Destructuring props
  ...
  const { PostStory } = useAppEndpoints();
  ...
  const nextScreenNavigate = async () => {
    try {
      ...
      const apiResponse = await PostStory(formData);
      if (apiResponse.ok && apiResponse !== null) {
        dispatch(StoriesActions.AddProfileStories(apiResponse.data.story));
