...
function ThemeProvider(props: ThemeProviderProps) {
  // Destructuring props
  const { navigationBarResponsive = true, children } = props;
  ...
  useEffect(() => {
    if (navigationBarResponsive) NavigationBar.setBackgroundColorAsync(theme.colors.background);
  }, []);

  // Light/Dark mode change listener
  useEffect(() => {
    Appearance.addChangeListener(({ colorScheme }) => ChangeMode(colorScheme));

    return () => Appearance.removeChangeListener(({ colorScheme }) => ChangeMode(colorScheme));
  }, []);
